...
**功能**：创建精美的、动画丰富的 HTML 演示文稿 —— 从零开始或转换 PowerPoint 文件。

**触发场景**：
- 需要创建演示文稿、幻灯片、Pitch Deck
- 提到"做PPT"、"生成幻灯片"、"网页演示"
- 需要将 PowerPoint 转换为网页格式

**核心特性**：
- **零依赖** —— 单 HTML 文件，内嵌 CSS/JS，无需 npm 或构建工具
- **视觉风格发现** —— 通过生成视觉预览让用户选择，而非抽象描述
- **PPT 转换** —— 将现有 PowerPoint 转换为网页，保留所有图片和内容
- **反 AI 平庸化** —— 精心设计 12 种独特风格，避免通用 AI 美学
- **12种精选风格** —— 包含暗黑主题（Bold Signal、Electric Studio 等）和浅色主题（Notebook Tabs、Swiss Modern 等）

**位置**：`frontend-slides/SKILL.md`

**来源**：来自 [zarazhangrui](https://github.com/zarazhangrui/frontend-slides)，遵循 MIT 许可证。

---

#### 2. 共情文章生成 (empathetic-article)
...
#### 6. Modern UI Designer (modern-ui-designer)
...
#### 1. Frontend Slides (frontend-slides)

**Purpose**: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files.

**Trigger Scenarios**:
- Need to create presentations, slideshows, pitch decks
- Mentions "make PPT", "generate slides", "web presentation"
- Need to convert PowerPoint to web format

**Key Features**:
- **Zero Dependencies** — Single HTML files with inline CSS/JS, no npm or build tools
- **Visual Style Discovery** — Generate visual previews for users to choose from instead of abstract descriptions
- **PPT Conversion** — Convert existing PowerPoint to web while preserving all images and content
- **Anti-AI-Slop** — 12 carefully curated distinctive styles avoiding generic AI aesthetics
- **12 Style Presets** — Dark themes (Bold Signal, Electric Studio, etc.) and light themes (Notebook Tabs, Swiss Modern, etc.)

**Location**: `frontend-slides/SKILL.md`

**Source**: From [zarazhangrui](https://github.com/zarazhangrui/frontend-slides), MIT licensed.

---

#### 2. Empathetic Article Generator (empathetic-article)
...
#### 3. Book Breakdown (book-breakdown)
...
#### 4. Truth Roundtable (truth-roundtable)
